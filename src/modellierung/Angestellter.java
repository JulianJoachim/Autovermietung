package modellierung;

/**
 * @author Julian Joachim s0574380<br>
 * @version Eclipse 2019-09 4.1.3 / Java 1.8.0_ 191<br>
 * @since 14.06.2020 zuletzt bearbeitet am 16.06.2020<br>
 *        Die Klasse Angestellten ist eine spezifischer Beobachter. Er überwacht
 *        die neuen Rechnungen.
 *
 */
public class Angestellter implements Beobachter {
	private String vorname = "Ange";
	private String nachname = "Stellter";

	/**
	 * Standardkonstruktor
	 * 
	 * @param vorname  Vorname des Angestellten
	 * @param nachname Nachname des Angestellten
	 */
	public Angestellter(String vorname, String nachname) {
		this.vorname = vorname;
		this.nachname = nachname;
	}

	/**
	 * Updatet die Informationen die er erhält, in dem Falle nur durch eine
	 * Konsolenausgabe die den Vornamen, Nachnamen und die Rechnungsnummer ausgibt,
	 * wenn der Rechnungsbetrag des Kunden 1000 überschreitet hat.
	 */
	@Override
	public void update(Rechnung r, Kunde k, String who) {
		if (who.equals("Angestellter")) {
			if (k.getTotalBetrag() >= 1000) {
				System.out.println("Angestellter " + vorname + " " + nachname
						+ " hat die Information erhalten, dass der Kunde zu Rechnung Nr. " + r.getRechnungsnummer()
						+ " zu viele Schulden hat.");
			}
		}
	}

}
